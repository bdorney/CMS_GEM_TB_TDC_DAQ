...
int main( int argc_, char * argv_[] ){
    ...
    for (auto iterVMEBoard = rSetup.m_map_vmeBoards.begin(); iterVMEBoard != rSetup.m_map_vmeBoards.end(); ++iterVMEBoard) {
        //Skip board if it's not a TDC
        if ( (*iterVMEBoard).second.m_vme_type != kVMETDC ) {continue;}
        
        cout<<"TDC " << (*iterVMEBoard).second.m_strBaseAddress;
        cout<< " FSR = " << std::stoul((*iterVMEBoard).second.m_strFullScaleRange, nullptr, 16 ) << endl;
        recoDigi.setTDCResolution( (*iterVMEBoard).second.m_strBaseAddress, 8.9 / std::stoul((*iterVMEBoard).second.m_strFullScaleRange, nullptr, 16 ) );
    }
